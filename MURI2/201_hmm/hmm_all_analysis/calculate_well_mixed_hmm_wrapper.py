...
for population in populations:
    ...
    try: #[WH] add try feature
        ...
        Pstate, Ls, p0, p1, LT = well_mixed_model.infer_hmm(A,D,num_iterations=10)#Pstate, Ls, p0, p1 = well_mixed_model.infer_hmm(A,D,num_iterations=10)  #[WH] now also ouput L_T
        ...
        LT_filename = parse_file.data_directory+("%s_well_mixed_state_timecourse_LT.txt" % population) #[WH] output L_T
        LT_file = open(LT_filename, "w")
        LT_file.write("%s" % "\n".join([str(l) for l in LT])) #[WH] output L_T
        LT_file.close() #[WH] output L_T
