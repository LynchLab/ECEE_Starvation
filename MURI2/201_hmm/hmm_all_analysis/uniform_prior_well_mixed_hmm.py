...
states = parse_file.well_mixed_hmm_states #well_mixed_hmm_states = {'A':0,'E':1,'F':2,'P':3}    
...
def infer_hmm(A, D, num_iterations=10):
    ...
    birth_rate = 1e-02 #[WH]A->P
    recurrence_rate = 1e-06 #[WH]E->A
    extinction_rate = 0.5 #[WH]P->E=ext_rate*ext_prob; P->F=ext_rate*(1-ext_prob); P->P=(1-ext_rate)
    ...
    Q = numpy.zeros((T,L,L)) # transition matrix #[WH] (timePoint, newState, oldState)
    F = numpy.zeros((M,T,L))*1.0 # forward probability table #[WH] (mutation, timePoint, State)
    B = numpy.zeros((M,T,L))*1.0 # backward probability table #[WH] (mutation, timePoint, State)
    ...
    LT = numpy.zeros(M)*1.0 #[WH]calculate L_T[20180419]
    ...
    for current_iteration in xrange(1,num_iterations+1):
        ...
        Q = numpy.zeros((T, L, L))  #[WH] Transition matirx Q is actually fixed
        ...
        V = numpy.zeros_like(logV)  #[WH] Emission matrix? (mutation,timePoint,State)
        
        #sys.stderr.write("Calculating emission probabilities...\n")  #[WH] Emission matirx V is updated due to the update of p0 and p1
        ...
        p0 = ancestral_error_numerators/(ancestral_error_denominators+(ancestral_error_denominators<1e-12)) #[WH] update p0
        p0 = numpy.clip(p0,min_error,epsE)
        p0 = numpy.outer(p0,numpy.ones(T))
        
            
        p1 = derived_error_numerators/(derived_error_denominators+(derived_error_denominators<1e-12)) #[WH] update p1
        p1 = numpy.clip(p1,min_error,epsF)
        p1 = numpy.outer(p1,numpy.ones(T))
        
    
        if current_iteration==num_iterations:
            # infer most likely sequence of states  #[WH] by Viterbi Algorithm
            #print F[:,T-1,:] #[WH]
            LT = numpy.squeeze(F[:,T-1,:].sum(axis=1)) #[WH] calculalate L_T by forward probablity
            ...
            print max_likelihood_table[:,-1,:] #[WH]
    ...
    return Pstate,Ls,p0,p1,LT#return Pstate,Ls,p0,p1 #[WH] update to output LT
