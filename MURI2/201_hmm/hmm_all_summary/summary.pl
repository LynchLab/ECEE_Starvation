...
open OUT, ">/Users/wei-chinho/Documents/MURI2/201_hmm/hmm_all_summary/summary.txt";
...
foreach my $p(@pop){
	...
	$CoverageLine = <DATA>;
	foreach my $i(1..5){
		my $notUsedLine = <HMM1>;   #times, hard_ns[0,:], hard_ns[1,:], hard_ns[2,:], hard_ns[3,:]
	...
	if ($cladeHMMResult{$p} >=0){
		...
		$CoverageLine = <DATA>;
		foreach my $i(1..9){
			my $notUsedLine = <HMM2>;   #times, fmajors, fminors, hard_ns[0,:], hard_ns[5:,:].sum(axis=0), 
...
# print header
print OUT "pop\tpop_type\tnum_TP\tout\tcoexist_length";
foreach my $t(@SNPTypeArray){
