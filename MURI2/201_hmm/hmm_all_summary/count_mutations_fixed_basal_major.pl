...
open OUT, ">/Users/wei-chinho/Documents/MURI2/201_hmm/hmm_all_summary/count_mutations_fixed_basal_major.txt"; #fixed in basal or major
...
@mutTypeList = ("synonymous", "nonsynonymous", "nonsense", "noncoding", "pseudogene", "intergenic", "indel", "sv");
...
print OUT "pop\tpop_type\tnum_TP\tout\tcoexist_length\tmut_type\tmut_count\ttotal_mut_count\tfixed_count\ttotal_fixed_count\n";
...
foreach my $p(@pop){
	...
	if ($cladeHMMResult{$p} !=1){
		...
		while (my $line1 = <DATA>){
			...
			foreach my $SNPstateIter(@temp2){
				if ($SNPstateIter == 2 || $SNPstateIter == 3){  #basal or major

					$fixedCount{$p}{$mutType} ++;
					$totalFixedCount{$p} ++;
					last;
				}
			}
	...
	foreach my $t(@mutTypeList){
		print OUT $p."\t".$pop2pop_type{($p%100)}."\t".$numT{$p}."\t".$cladeHMMResult{$p}."\t".$coexistLength{$p};
		print OUT "\t".$t."\t".(0+$mutCount{$p}{$t})."\t".(0+$totalMutCount{$p});
		print OUT "\t".(0+$fixedCount{$p}{$t})."\t".(0+$totalFixedCount{$p})."\n"
	} 
}		
