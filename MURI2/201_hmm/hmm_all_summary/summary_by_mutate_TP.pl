...
open OUT, ">/Users/wei-chinho/Documents/MURI2/201_hmm/hmm_all_summary/summary_by_mutate_TP.txt";
...
foreach my $p(@pop){
	...
	while (my $line1 = <DATA>){
		...
		$count{$p}{$mutateTP}{$SNPType.$SNPstate} ++;
	}
	close DATA;
	close HMM1;
	...
	if ($cladeHMMResult{$p} >=0){
		...
		foreach my $t(@SNPTypeArray){
			foreach my $s(1..8){
				foreach my $t(1..($numTP-1)){
					$mutateTP = $timePointArray[$t];
					$count{$p}{$mutateTP}{$t.$s} = -1;
				}
			}
		}
	}	
...
# print header
print OUT "pop\tpop_type\tnum_TP\tout\tcoexist_length\tmutate_TP";
foreach my $t(@SNPTypeArray){
...
foreach my $p(@pop){
	foreach my $tp(@{$pop2TimePointArray{$p}}){
		if($tp > 0){
			print OUT $p."\t".$pop2pop_type{($p%100)}."\t".$numT{$p}."\t".$cladeHMMResult{$p}."\t".$coexistLength{$p}."\t".$tp;
			foreach my $t(@SNPTypeArray){
				foreach my $s(@SNPStateArray){
