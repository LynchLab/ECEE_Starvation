# usr/bin/perl
# temporal data of everything

open OUT, ">/Users/wei-chinho/Documents/MURI2/201_hmm/hmm_all_summary/summary_temporal.txt";
...
# count the status of SNPs in every available time point
my %count; #{$p}{$t}{$SNPType.$SNPstate} 
...
foreach my $p(@pop){
	...
	while (my $line1 = <DATA>){
		...
		#check the status at the last time point in HMM1
		if ($SNPType eq "synonymous"){
			$SNPType = "S";
		}elsif ($SNPType eq "nonsynonymous"){
			$SNPType = "N";
		}elsif ($SNPType eq "intergenic"){
			$SNPType = "I";
		}

		foreach my $ti(0..($numTP-1)){

			my $tp = $timePointArray[$ti];
			
			#check the status at the every time point in HMM1
			my $SNPstate = "";
			if($temp2[$ti] == 1){
				$SNPstate = "E";
			}elsif($temp2[$ti] == 2){
			...
			$count{$p}{$tp}{$SNPType.$SNPstate} ++;
		}
	}
	close DATA;
	close HMM1;

	if ($cladeHMMResult{$p} >=0){
		...
		while (my $line1 = <DATA>){
			...
			#check the status at the every time point in HMM2
			if ($SNPType eq "synonymous"){
				$SNPType = "S";
			}elsif ($SNPType eq "nonsynonymous"){
				$SNPType = "N";
...
# print header
print OUT "pop\tpop_type\tnum_TP\tout\tTP";
foreach my $t(@SNPTypeArray){
	foreach my $s(@SNPStateArray){
		print OUT "\t".$t.$s;
	}
}
foreach my $t(@SNPTypeArray){
...
foreach my $p(@pop){
	foreach my $tp( sort{$a <=> $b} keys %{$count{$p}}){
		print OUT $p."\t".$pop2pop_type{($p%100)}."\t".$numT{$p}."\t".$cladeHMMResult{$p}."\t".$tp;
		foreach my $t(@SNPTypeArray){
			foreach my $s(@SNPStateArray){
				print OUT "\t".(0+$count{$p}{$tp}{$t.$s});
			}
		}
		foreach my $t(@SNPTypeArray){
			foreach my $s(1..8){
