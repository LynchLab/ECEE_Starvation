...
open OUT, ">/Users/wei-chinho/Documents/MURI2/201_hmm/hmm_all_summary/count_mutations_fixed_any.txt";  #fixed in any clade
	...
	if ($cladeHMMResult{$p} !=-1){
		...
		while (my $line1 = <DATA>){
			...
			foreach my $SNPstateIter(@temp2){
				if ($SNPstateIter == 2 || $SNPstateIter == 3|| $SNPstateIter == 4){
